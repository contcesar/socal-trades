// Contact form submission. Verifies Turnstile, then emails the message to the
// admin via Resend. When Resend isn't configured, logs it and still confirms.
//
...
export default async (req) => {
  ...
  if (key && from && to) {
    try {
      ...
      if (!res.ok) {
        console.error("Resend contact send failed:", res.status, await res.text());
  ...
  // Not configured: log so it is recoverable, still confirm.
